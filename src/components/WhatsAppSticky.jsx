...
const WhatsAppStickyV2 = () => {
  const { t } = useTranslation()
  const { isRTL } = useRTL()
  
  const handleWhatsAppClick = () => {
    const phoneNumber = "212777298195"
    const message = t('whatsapp.defaultMessage')
    
    // Méthode 1: URL directe
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`
    
    // Méthode 2: Intent pour mobile
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent)
    
    if (isMobile) {
      // Essayer l'intent WhatsApp d'abord
      const intentUrl = `whatsapp://send?phone=${phoneNumber}&text=${encodeURIComponent(message)}`
  ...
  return (
    ...
    >
      ...
      {/* Pulse Animation */}
      <div className="absolute inset-0 bg-green-400 rounded-full whatsapp-pulse pointer-events-none"></div>
      
      {/* Glow Effect */}
      <div className="absolute inset-0 bg-green-300 rounded-full animate-pulse opacity-20 pointer-events-none"></div>
    </div>
  )
}

export default WhatsAppStickyV2
